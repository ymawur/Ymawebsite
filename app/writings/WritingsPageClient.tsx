        ...
        <Container>
          <div className="grid md:grid-cols-2 gap-6">
            {writings.map((writing) => (
              <Card key={writing.slug} href={`/writings/${writing.slug}`} className="h-full">
                <p className="text-sm font-medium text-accent-700 mb-3">{writing.kicker}</p>
                <h2 className="text-xl font-semibold text-gray-900 mb-2">{writing.title}</h2>
                <p className="text-sm text-gray-600 leading-6">{writing.summary}</p>
              </Card>
            ))}
          </div>
        </Container>
